...
def import_data(train_dataframe, dev_dataframe, test_dataframe):
    ...
    #New Include all 12 initial factors (Hour ; Day ; Month ; Year ; Cloud Coverage ; Visibility ; Temperature ; Dew Point ;
...
def build_model(init_type='glorot_uniform', optimizer='adam'):
    ...
    #layers = [12, 64, 64, 1, 1]
    layers = [12, 64, 64, 1]
    model.add(keras.layers.LSTM(layers[0],input_shape = (None,12),return_sequences=True))
    model.add(keras.layers.Dropout(0.2))

    model.add(keras.layers.LSTM(layers[1],kernel_initializer = init_type,return_sequences=True
        #bias_initializer = 'zeros'
    ))
    model.add(keras.layers.Dropout(0.2))

    model.add(Dense(layers[2], activation='tanh',kernel_initializer=init_type,input_shape = (None,1)))
...
def main():
    ...
    #adaboost = AdaBoostClassifier(base_estimator=build_model, learning_rate=0.01, algorithm='SAMME.R')
